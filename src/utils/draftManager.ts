...
// Define types for Draft and Version
...
// Create a new draft
export async function createDraft(postData: DraftData): Promise<string> {
    try {
...
export async function updateDraft(draftId: string, postData: DraftData): Promise<number> {
    try {
...
export async function createDraftShare(draftId: string, expiresIn: number = 7): Promise<string> { // Default 7 days
    try {
...
export async function verifyShareToken(token: string): Promise<{ valid: boolean; draftId?: string; reason?: string }> {
    try {
        ...
        const shareData = shareDoc.data() as DraftShare;
        const now = new Date();
        const expiresAt = shareData.expiresAt instanceof Timestamp ? shareData.expiresAt.toDate() : shareData.expiresAt;
...
// Revoke a share link
export async function revokeDraftShare(token: string): Promise<boolean> {
    try {
        const shareRef = doc(db, COLLECTIONS.DRAFT_SHARES, token);
...
export async function setDraftExpiration(draftId: string, expiresIn: number = 30): Promise<boolean> { // Default 30 days
    try {
        const expiresAt = new Date();
        expiresAt.setDate(expiresAt.getDate() + expiresIn);
