...
interface MediaLibraryProps {
    onSelect: (item: MediaItemType | MediaItemType[]) => void;
    multiple?: boolean;
...
export default function MediaLibrary({ onSelect, multiple = false, initialSelected = [], onClose }: MediaLibraryProps) {
    const [mediaItems, setMediaItems] = useState<MediaItemType[]>([]);
    const [loading, setLoading] = useState(true);
    const [uploading, setUploading] = useState(false);
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedItems, setSelectedItems] = useState<MediaItemType[]>(initialSelected);
    ...
    const handleUpdateMetadata = async (mediaId: string, metadata: Partial<MediaItemType>) => {
        try {
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        name="altText"
                                        defaultValue={editingItem.altText} // Assuming altText exists in MediaItemType but not explicitly defined yet
                                        className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600"
                                    />
                                </div>
                                <div>
